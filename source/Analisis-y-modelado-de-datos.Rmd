...
```{r, echo=FALSE}
# Reverse one hot encoding para el plot de las series temporales
# Seleccionamos las columnas location
locations <- grep("^Location.[a-zA-Z0-9]*", colnames(zonas[[3]]), value = TRUE)
...
Loc_2 <- zonas[[3]] 
Loc_2["Location"] <- col_vec %>% as.factor()
Loc_2 <- Loc_2 %>% filter(.,Date>= as.Date("2017-01-01"))%>% select(.,-all_of(locations))
