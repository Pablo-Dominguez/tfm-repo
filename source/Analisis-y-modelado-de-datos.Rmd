...
A continuación, buscamos comprobar si la serie temporal está completa y sin repetidos para cada valor de la variable *Location*. En la tabla \ref{tab:Tab-3}, se muestran las variables conteniendo la siguiente información.
...
dates_df <- date_range()
dates_df %>% kbl(., booktabs = T, caption = "Continuidad de las series temporales") %>% kable_styling(latex_options = c("striped","HOLD_position","scale_down"))
...
dates_df <- date_range()
dates_df %>% slice(., 1:12) %>% kbl(., booktabs = T, caption = "Series temporales desde el 2014-01-01 hasta el 2017-06-24") %>% kable_styling(latex_options = c("striped","HOLD_position","scale_down"))
...
```{r Fig-01, out.width="75%", echo=FALSE, fig.cap="\\centering Mapa de las zonas climáticas de Australia - \\footnotesize{Fuente: Australian Building Codes Board}",fig.align='center'}
