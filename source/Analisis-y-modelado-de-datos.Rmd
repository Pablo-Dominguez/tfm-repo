...
date_range <- function(){
  ...
  for(city in levels(db$Location)){tryCatch({
...
get_outliers <- function(){
  ...
  for(i in 1:length(zonas)){
    ...
    plots_list <- c(plots_list,paste0("zona"+as.character(i))=ggarrange(plotlist =  ps, nrow = 4,ncol = 3,common.legend = TRUE))
    dev.off()
  }
  return(plots_list)
}
