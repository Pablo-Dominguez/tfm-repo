...
Nos disponemos a continuación a visualizar las series temporales de las variables numéricas de la zona 1. El objetivo de esta visualización es apoyar la descomposición que haremos más tarde de las series temporales, aportándo variables tipo ventana con información de estadísticos previos.
...
```{r, echo=FALSE,out.width="90%", fig.cap="\\centering Muestra de las series temporales por `Location` para cada variable de la zona 1",fig.align='center'}
...
```{r Fig-11,echo=FALSE,fig.cap="Serie temporal de la variable `MinTemp` en la zona 1, según el valor de `Location` ", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-12,echo=FALSE,fig.cap="Serie temporal de la variable `MaxTemp` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-13,echo=FALSE,fig.cap="Serie temporal de la variable `Rainfall` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-14,echo=FALSE,fig.cap="Serie temporal de la variable `Humidity9am` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-15,echo=FALSE,fig.cap="Serie temporal de la variable `Humidity3pm` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-16,echo=FALSE,fig.cap="Serie temporal de la variable `Temp9am` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-17,echo=FALSE,fig.cap="Serie temporal de la variable `Temp3pm` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-18,echo=FALSE,fig.cap="Serie temporal de la variable `WindGustDir\\_x` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-19,echo=FALSE,fig.cap="Serie temporal de la variable `WindGustDir\\_y` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-20,echo=FALSE,fig.cap="Serie temporal de la variable `WindDir9am\\_x` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-21,echo=FALSE,fig.cap="Serie temporal de la variable `WindDir9am\\_y` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-22,echo=FALSE,fig.cap="Serie temporal de la variable `WindDir3pm\\_x` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
```{r Fig-23,echo=FALSE,fig.cap="Serie temporal de la variable `WindDir3pm\\_y` en la zona 1, según el valor de `Location`", message=FALSE,results='hide', out.height="40%",fig.align = 'center', fig.pos="H"}
...
Para tratar la dimensión `Tiempo`, vamos a crear una serie de columnas adicionales para cada variable. Por un lado, vamos a dedicar esta sección descomponer las series temporales de cada variable, obteniendo una información de la tendencia o `trend` y de la estacionalidad o `seasonality` de la serie temporal.

```{r, eval=FALSE}
locations <- grep("^Location.[a-zA-Z0-9]*", colnames(zonas[[1]]), value = TRUE)
for(city_index in which(colnames(zonas[[1]]) %in% locations)){
    db_filtered <- zonas[[1]] %>% filter(if_any(city_index, ~ . == 1)) %>% select(.,-c(locations,"RainToday","RainTomorrow"))
    city_ts_decomposition <- decompose(birthsComp)
}
...
Con esta idea en mente, para cada zona vamos a realizar una selección de variables siguiendo el algoritmo *step* descrito en @MASS. Este algoritmo realiza una regresión lineal sencilla, añadiendo variables al modelo al tiempo que calculando el aporte de cada variable sobre la predicción siguiendo el *criterio de información de Akaike*. Con esto, en cada iteración se añaden o eliminan variables hasta que obtenemos una selección de las variables más relevantes para la predicción.
...
```{r step, echo=FALSE,message=FALSE,results='hide'}

top_names_list <- list()
for(i in 1:length(last_dbs)){
  lm1 <- lm(RainTomorrow ~ ., data = last_dbs[[i]])
  #summary()
  slm1 <- step(lm1)
  topnames <- c(names(slm1$coefficients)[2:16],"RainTomorrow")
  top_names_list[[paste0("zona",as.character(i))]] <- topnames
  # summary(slm1)
  # slm1$anova
}

db_vars <- data.frame(matrix(unlist(top_names_list), nrow=length(top_names_list), byrow=TRUE)) %>% lapply(.,function(x) gsub("_", "\\\\_", x)) %>% as.data.frame()%>% unite("Imp. vars", X1:X15,sep = " ")
rownames(db_vars) <- names(top_names_list) 
# lapply(.,function(x) str_replace_all(.,"_","\\\\_"))


```

```{r Tab-13,echo=FALSE,message=FALSE}

# function(x) paste(c(str_split(x, " ")[[1]][1:8] %>% paste(., collapse=' '),str_split(x, " ")[[1]][9:15] %>% paste(., collapse=' ')),collapse="\n")
# 
db_vars["Imp. vars"] <- db_vars %>% mutate_at(.,"Imp. vars",function(x){ paste(c(str_split(x, " ")[[1]][1:8] %>% paste(., collapse=' '),str_split(x, " ")[[1]][9:15] %>% paste(., collapse=' ')),collapse="\n")})




db_vars %>% kable(., booktabs = T,caption = "15 variables más importantes para cada zona según el AIC.",escape = FALSE) %>% kable_styling(latex_options = c("striped","hold_position","scale_down")) %>% row_spec(0,align = 'c')

```


```{r echo=FALSE}
# Seleccionamos las columnas para cada variable
# 
for(i in 1:length(last_dbs)){
  last_dbs[[i]] <- last_dbs[[i]] %>% select(.,top_names_list[[i]])
  db_to_save <- as.data.frame(last_dbs[[i]])
  write.csv(db_to_save,paste0("../db/processed_dbs/data_zona",as.character(i),".csv"),row.names = FALSE)
}
