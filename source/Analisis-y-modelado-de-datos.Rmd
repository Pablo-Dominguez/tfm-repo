...
sample_cols <- paste0(columnas_numericas,"_mean7") %>% sample(5)
...
**Con las numéricas normalizamos y hacemos el step**
...
for(i in 1:length(last_dbs)){
  ...
  #summary()
  slm1 <- step(lm1)
  topnames <- names(slm1$coefficients)[2:16]
...
for(i in 1:length(last_dbs)){
  ...
  save(db_to_save,file=paste0("../db/processed_dbs/data_zona",as.character(i),".Rda"))
}
